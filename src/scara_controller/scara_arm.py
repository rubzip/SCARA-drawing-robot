...
class ScaraArm:
    ...
    def get_jacobian_inverse(self, q: np.ndarray, use_pseudo: bool = True) -> np.ndarray:
        q = self.__get_q(q)
    ...
    def get_q_dot(self, desired_p: np.ndarray, current_q: np.ndarray = None, k: float = 1., normalize: bool = False) -> np.ndarray:
        ...
        J_inv = self.get_jacobian_inverse(q)
        current_p = self.get_pos(q)
        p_dot = (desired_p - current_p) * k
        q_dot = J_inv @ p_dot

        if normalize and self.q_dot_max:
