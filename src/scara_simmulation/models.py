import math as m
import numpy as np
from src.scara_kinematics.scara_kinematics import ScaraKinmeatics


class ScaraModel:
    """SCARA 2D arm model for simulation purposes."""
    def __init__(self, length_1: float, length_2: float, center: tuple[float, float] = None):
...
class ScaraSimulator:
    ...
    def __init__(self, length_1: float, length_2: float,
        ...
        self.kinematics = ScaraKinmeatics(length_1, length_2, q, q_dot_max=q_dot_max)
        self.arm = ScaraModel(length_1, length_2, center=center)
    ...
    def _is_target_achived(self, eps: float = 0.1) -> bool:
